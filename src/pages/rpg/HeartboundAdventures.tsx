...
import { MeadowHavenCanvas } from '@/components/heavy-games/heartbound/MeadowHavenCanvas';
import { getDisplayNameFromEmail } from '@/lib/auth-config';
import { Heart, Map, Home as HomeIcon, Users, Sparkles, Lock, ArrowLeft, Star, ChevronRight, Play, Flower2 } from 'lucide-react';

// Sprite colour per player
const SPRITE_COLOR: Record<string, string> = {
  'sinharonitraj@gmail.com':     '#60a5fa',  // blue for Ronit
  'radhikadidwania567@gmail.com':'#f472b6',  // pink for Radhika
  'shizzandsparkles@gmail.com':  '#a78bfa',  // purple for Shizz
};
...
export const HeartboundAdventures: React.FC = () => {
  ...
  // ── Room join handler from InviteModal ──────────────────────────
  const handleRoomReady = useCallback((rid: string) => {
    setRoomCode(rid);
    setView('meadow');
  }, []);

  // ── Flower collect → bond XP ────────────────────────────────────
  const handleCollect = useCallback((total: number) => {
    setFlowerCount(total);
    setBondXP(prev => Math.min(100, prev + 5)); // +5 XP per flower
  }, []);

  // ── Solo play (no multiplayer room) ─────────────────────────────
  const handleSolo = () => {
    setRoomCode(`solo-${Date.now()}`);
    setView('meadow');
  };

  // ─────────────────────────────────────────────────────────────────
  // MEADOW HAVEN GAME VIEW
  // ─────────────────────────────────────────────────────────────────
  if (view === 'meadow') {
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-4">
        {/* Stats bar above canvas */}
        <div className="w-full max-w-[960px] flex items-center justify-between mb-3 px-2">
          <div className="flex items-center gap-2 text-white/80 text-sm">
            <span className="text-xl">🌿</span>
            <span className="font-bold">Meadow Haven</span>
            {roomCode && !roomCode.startsWith('solo') && (
              <span className="bg-white/10 px-2 py-0.5 rounded-full text-xs">Room: {roomCode}</span>
            )}
          </div>
          <div className="flex items-center gap-4 text-sm text-white/70">
            <span>🌸 {flowerCount} collected</span>
            <span>💕 {bondXP} XP</span>
          </div>
        </div>
        ...
        <MeadowHavenCanvas
          roomCode={roomCode!}
          myColor={myColor}
          onBack={() => { setView('hub'); setRoomCode(null); }}
          bondXP={bondXP}
          onCollect={handleCollect}
        />

        <p className="text-white/30 text-xs mt-3">
          Collect 🌸 flowers to earn Bond XP • More islands coming soon
        </p>
      </div>
    );
  }
  ...
  if (view === 'invite') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-purple-50 dark:from-pink-950 dark:via-rose-950 dark:to-purple-950 flex items-center justify-center">
        <InviteModal
          gameType="HeartboundAdventures"
          onClose={() => setView('hub')}
          onReady={(rid) => handleRoomReady(rid)}
        />
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-4">
          <button
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold text-lg px-8 py-4 rounded-2xl shadow-xl hover:shadow-pink-300/50 hover:scale-105 transition-all"
          >
            <Users size={20} /> Play with Partner
          </button>
          <button
            onClick={handleSolo}
            className="inline-flex items-center gap-2 glass-btn text-gray-700 dark:text-gray-200 font-semibold text-lg px-8 py-4 rounded-2xl hover:scale-105 transition-all"
          >
            <Play size={18} /> Explore Solo
          </button>
        </div>
        <p className="text-sm text-gray-400">Island 1 — Meadow Haven — is playable now 🌿</p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            { icon: <HomeIcon size={24} className="text-white" />,         color: 'from-purple-500 to-violet-500', title: 'Dream Home Builder', desc: 'Decorate your shared home with furniture and items discovered on your adventures.' },
        ...
        <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-2 gap-10 text-white">
          <div>
            ...
            <div className="space-y-3">
              {[
                { g: '🌃 Sky: Children of the Light', d: 'Emotional co-op connection and discovery' },
                { g: '🏔️ Minecraft',                  d: 'Shared world exploration and building' },
                { g: '🏡 Animal Crossing',              d: 'Cozy customization and life sim vibes' },
              ].map(x => (
                <div key={x.g} className="bg-white/10 rounded-xl p-4">
                  <p className="font-semibold">{x.g}</p>
                  <p className="text-sm text-pink-100">{x.d}</p>
                </div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-3">
              {[
                { phase:'1', title:'Meadow Haven (Live!)',    desc:'Canvas world, real-time co-op movement, flower collection', done: true },
                { phase:'2', title:'NPCs & Interactions',     desc:'Friendly deer, fairies, proximity prompts, more collectibles', done: false },
                { phase:'3', title:'Home Builder',            desc:'Shared interior, drag-drop furniture, Firebase-persisted layout', done: false },
              ].map(s => (
                <div key={s.phase} className={`rounded-xl p-4 flex items-start gap-4 ${s.done ? 'bg-white/20' : 'bg-white/10'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${s.done ? 'bg-yellow-400 text-gray-900' : 'bg-white/20 text-white'}`}>
                    {s.done ? '✓' : s.phase}
                  </div>
                  <div>
                    <p className="font-semibold">{s.title}</p>
                    <p className="text-sm text-pink-100">{s.desc}</p>
                  </div>
                </div>
              ))}
            </div>
