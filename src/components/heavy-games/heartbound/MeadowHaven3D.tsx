 ...
 * 1. Escape key — browser ALWAYS exits fullscreen before JS fires.
 *    Solution: listen to `fullscreenchange`, detect if the exit was caused
 *    by Escape (via a flag set on keydown), open the menu, and
 *    immediately re-request fullscreen so the game stays full-screen.
 * 2. GameMenu — complete visual redesign: gradient header, glowing
 *    sections, animated tabs, prominent "Exit to Hub" danger button.
 * 3. Enter button — requests fullscreen on the containerRef element
 *    directly from the button click handler (user-gesture required).
 */
import React, {
    ...
    <group ref={rootRef}>
      ...
      <group ref={lLegRef} position={[-0.13, 0.35, 0]}>
        <Cylinder args={[0.095, 0.10, 0.55, 8]} position={[0, -0.28, 0]} castShadow>
          <meshStandardMaterial color={darkOutfit} roughness={0.85} metalness={0} /></Cylinder>
        <Sphere args={[0.11, 8, 6]} position={[0, -0.59, 0.04]} castShadow>
          <meshStandardMaterial color="#1e1b18" roughness={0.9} metalness={0} /></Sphere>
      </group>
      ...
      <Cylinder args={[0.225, 0.225, 0.08, 12]} position={[0, 0.70, 0]} castShadow>
        <meshStandardMaterial color={skinTone} roughness={0.80} metalness={0} /></Cylinder>
      <group ref={lArmRef} position={[-0.29, 0.58, 0]}>
        <Cylinder args={[0.075, 0.085, 0.52, 8]} position={[0, -0.26, 0]} castShadow>
          <meshStandardMaterial color={outfitColor} roughness={0.75} metalness={0} /></Cylinder>
        <Sphere args={[0.085, 8, 6]} position={[0, -0.55, 0]} castShadow>
          <meshStandardMaterial color={skinTone} roughness={0.80} metalness={0} /></Sphere>
      </group>
      ...
      <Sphere args={[0.29, 16, 12]} position={[0, 1.15, 0]} castShadow>
        <meshStandardMaterial color={skinTone} roughness={0.75} metalness={0} /></Sphere>
      ...
      {hairStyle === 'short' && (
        <Sphere args={[0.31, 12, 10]} position={[0, 1.28, -0.04]} castShadow>
          <meshStandardMaterial color={hairColor} roughness={0.9} metalness={0} /></Sphere>
      )}
      {hairStyle === 'long' && (<>
        <Sphere args={[0.30, 12, 10]} position={[0, 1.28, -0.04]} castShadow>
          <meshStandardMaterial color={hairColor} roughness={0.9} metalness={0} /></Sphere>
        <Cylinder args={[0.14, 0.10, 0.55, 8]} position={[0, 0.95, -0.22]} castShadow>
          <meshStandardMaterial color={hairColor} roughness={0.9} metalness={0} /></Cylinder>
      </>)}
      {hairStyle === 'bun' && (<>
      ...
      {accessory === 'hat' && (
        <group position={[0, 1.48, 0]}>
          <Cylinder args={[0.36, 0.36, 0.07, 16]}>
            <meshStandardMaterial color="#1e1b18" roughness={0.85} metalness={0} /></Cylinder>
...
function RemoteAvatar({ player }: { player: PlayerState }) {
  ...
  return (
    <SkyCharacter config={cfg} name={player.name} isMe={false}
...
function Pond() {
  ...
  const lilyPads=useMemo(()=>[
    {pos:[1.2,0.08,-1.4] as [number,number,number],i:0},{pos:[-1.8,0.08,0.8] as [number,number,number],i:1},
    {pos:[0.5,0.08,2.1] as [number,number,number],i:2},{pos:[-2.3,0.08,-1.2] as [number,number,number],i:3},
    {pos:[2.5,0.08,1.0] as [number,number,number],i:4},{pos:[-0.7,0.08,-2.5] as [number,number,number],i:5},
  ],[]);
  return (
    <group>
...
function Fireflies() {
  ...
  const positions=useMemo(()=>{
    ...
    for(let i=0;i<count;i++){const a=Math.random()*Math.PI*2,r=1.8+Math.random()*5.5;arr[i*3]=Math.cos(a)*r;arr[i*3+1]=0.4+Math.random()*2.2;arr[i*3+2]=Math.sin(a)*r;}
    return arr;
  },[]);
  ...
  useFrame(({clock})=>{
    ...
    for(let i=0;i<count;i++){attrRef.current.setXYZ(i,orig[i*3]+Math.sin(t*0.55+i*1.35)*0.42,orig[i*3+1]+Math.sin(t*0.85+i*0.75)*0.28,orig[i*3+2]+Math.cos(t*0.48+i*1.12)*0.42);}
...
function MovementController({keysRef,posRef,movingRef,facingRef,onPublish,blockedRef}:{
  keysRef:React.MutableRefObject<Set<string>>;posRef:React.MutableRefObject<THREE.Vector3>;
  movingRef:React.MutableRefObject<boolean>;facingRef:React.MutableRefObject<number>;
  onPublish:(x:number,z:number,moving:boolean)=>void;blockedRef:React.MutableRefObject<boolean>;
}) {
  useFrame(()=>{
...
function Scene({myEmail,myName,avatarConfig,onCollect,onBondXP,nearbyNPCRef,setNearbyNPC,blockedRef,posRef,movingRef,facingRef,keysRef}:{
  ...
  return (
    <>
      ...
      <SkyCharacter config={avatarConfig} name={myName} isMe={true}
    ...
    <div className="absolute inset-0 z-30 flex items-center justify-center pointer-events-auto"
      ...
      <div className="w-full max-w-lg mx-3 rounded-3xl overflow-hidden shadow-2xl"
        ...
        <div className="relative px-6 py-5"
          ...
          <button onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition text-lg font-bold">
            ✕
          </button>
        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={it.label}
                    className={`rounded-2xl p-4 flex flex-col items-center gap-1.5 border transition ${
                      it.count>0?'border-white/15':'border-white/5 opacity-40'
                    }`}
        ...
        <div className="px-5 pb-5 flex gap-3"
          ...
          {/* Exit to Hub — prominent danger button */}
...
const HAIR_STYLE_LABELS:Record<AvatarConfig['hairStyle'],string>={short:'Short',long:'Long',bun:'Bun',none:'None'};
...
const ACCESSORY_LABELS:Record<AvatarConfig['accessory'],string>={none:'None',hat:'🎩 Hat',crown:'👑 Crown',halo:'😇 Halo'};

function AvatarCreator({onEnter,myColor}:{onEnter:(cfg:AvatarConfig)=>void;myColor:string}) {
  const [cfg,setCfg]=useState<AvatarConfig>({...DEFAULT_AVATAR,outfitColor:myColor});
  const set=<K extends keyof AvatarConfig>(key:K,val:AvatarConfig[K])=>setCfg(prev=>({...prev,[key]:val}));
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center"
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-2xl px-4">
        <div className="w-full md:w-56 flex-shrink-0 rounded-2xl overflow-hidden"
          style={{height:'280px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)'}}>
          <Canvas camera={{position:[0,0.9,2.4],fov:52}} style={{width:'100%',height:'100%'}}>
            ...
            <Suspense fallback={null}>
              <group position={[0,-0.85,0]}>
                <SkyCharacter config={cfg} name="" isMe={false} staticPos={[0,0,0]}/>
              </group>
            </Suspense>
            <OrbitControls enableZoom enablePan={false} minDistance={1.5} maxDistance={4.0}
              minPolarAngle={Math.PI/4} maxPolarAngle={Math.PI/1.8}
              autoRotate autoRotateSpeed={1.5} target={[0,0.1,0]}/>
          </Canvas>
          <div className="text-center text-white/25 text-xs py-1"
        ...
        <div className="flex-1 space-y-4 overflow-y-auto max-h-[60vh] pr-1">
          {[
            {label:'Skin Tone',    items:SKIN_TONES,   prop:'skinTone'   as const, type:'color' as const},
            {label:'Hair Colour',  items:HAIR_COLORS,  prop:'hairColor'  as const, type:'color' as const},
            {label:'Outfit Colour',items:OUTFIT_COLORS,prop:'outfitColor'as const, type:'color' as const},
          ].map(({label,items,prop,type})=>(
            <div key={prop}>
              <p className="text-white/60 text-xs font-semibold mb-2 uppercase tracking-wider">{label}</p>
              <div className="flex flex-wrap gap-2">
                {items.map(c=>(
                  <button key={c} onClick={()=>set(prop,c)}
                    className="w-8 h-8 rounded-full border-2 transition"
                    style={{background:c,borderColor:cfg[prop]===c?'white':'transparent'}}/>
                ))}
              </div>
            </div>
          ))}
      ...
      <button onClick={()=>onEnter(cfg)}
        ...
        style={{background:'linear-gradient(135deg,#6366f1,#ec4899)'}}>Enter Meadow Haven 🌿</button>
    </div>
  );
}
...
export const MeadowHaven3D: React.FC<Props> = ({myColor,onBack,bondXP,onCollect,onBondXP}) => {
  ...
  // ── FIX: track whether ESC caused the fullscreen exit ──
  ...
  useEffect(()=>{
    ...
    const down = (e:KeyboardEvent) => {
      ...
      if (e.key==='m'||e.key==='M') {
        if (hasD) closeDialogue(); else setMenuOpen(p=>!p);
        return;
      }
