...
const checkWinner = (board: (string | null)[][], row: number, col: number, player: string) => {
  ...
  for (let r = 0; r <= ROWS - 4; r++) for (let c = 0; c <= COLS - 4; c++) if ([0,1,2,3].every(i => board[r+i][c+i] === player)) return [[r,c],[r+1,c+1],[r+2,c+2],[r+3,c+3]];
  for (let r = 0; r <= ROWS - 4; r++) for (let c = 3; c < COLS; c++) if ([0,1,2,3].every(i => board[r+i][c-i] === player)) return [[r,c],[r+1,c-1],[r+2,c-2],[r+3,c-3]];
  return null;
};
...
export const Connect4: React.FC<{ sessionId?: string }> = ({ sessionId }) => {
  ...
  const { gameState, updateGameState, loading } = useRealtimeGame<Connect4GameState>(sessionId || 'connect4-game', 'connect4', initialState);
  ...
  const handleColumnClick = (col: number) => {
    ...
    const cells = checkWinner(newBoard, row, col, user?.email || '');
    const full  = newBoard[0].every(c => c !== null);
    if (cells) updateGameState({ ...gameState, board: newBoard, winner: user?.email || '', winningCells: cells, status: 'finished' });
    else if (full) updateGameState({ ...gameState, board: newBoard, isDraw: true, status: 'finished' });
    else {
      const next = gameState.currentPlayer === gameState.players.player1 ? gameState.players.player2 : gameState.players.player1;
  ...
  const isWinning = (r: number, c: number) => gameState?.winningCells?.some(([wr, wc]) => wr === r && wc === c) || false;

  if (loading) return <div className="flex items-center justify-center min-h-screen"><LoadingSpinner /></div>;

  const isP1 = gameState?.players.player1 === user?.email;
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {gameState?.status === 'waiting' && (
          <div className="glass-card p-8 text-center">
            ...
            <button onClick={startGame} className="bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold py-3 px-8 rounded-xl transition hover:scale-105">
              Start Game
            </button>
          </div>
        )}
        ...
        {gameState && gameState.status !== 'waiting' && (
          <div className="space-y-4">
            <div className="glass-card p-4 text-center">
              ...
              {gameState.status === 'finished' && (
                <p className="font-bold text-xl text-gray-900 dark:text-white">
                  {gameState.isDraw ? "🤝 Draw!" : gameState.winner === user?.email ? '🏆 You Win!' : '💔 You Lost!'}
                </p>
              )}
            </div>
                  ...
                  <div key={c} className="flex justify-center">
                    <div className={`w-4 h-4 rounded-full transition-all duration-200 ${
                      hoveredCol === c && isMyTurn && gameState.status === 'active'
                        ? `bg-gradient-to-b ${myColor} opacity-80`
                        : 'opacity-0'
                    }`} />
                  </div>
                ))}
            ...
            {gameState.status === 'finished' && (
              <button onClick={resetGame} className="w-full bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold py-3 rounded-xl transition hover:scale-[1.02]">
                Play Again 💕
              </button>
            )}
          </div>
