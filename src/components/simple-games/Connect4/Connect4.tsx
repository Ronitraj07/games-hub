...
export const Connect4: React.FC<Connect4Props> = ({ sessionId }) => {
  ...
  const { gameState, updateGameState, loading, error } = useRealtimeGame<Connect4GameState>(
    sessionId || 'connect4-game',
    'connect4',
    initialState
  );
  ...
  const startGame = () => {
    updateGameState({
      ...
      players: {
        player1: user?.email || '',
        player2: user?.email === 'sinharonitraj@gmail.com' ? 'radhikadidwania567@gmail.com' : 'sinharonitraj@gmail.com'
      },
      status: 'active'
    });
  };
            ...
            <div className="bg-blue-50 dark:bg-gray-800 rounded-lg p-4 text-center">
              ...
              <div className="flex items-center justify-center gap-4 mt-2">
                <div className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-full ${myColor}`} />
                  <span className="text-sm text-gray-700 dark:text-gray-300">You</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-full ${opponentColor}`} />
                  ...
                  row.map((cell, colIdx) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!isMyTurn || rowIdx !== ROWS - 1}
                        className={`w-12 h-12 md:w-16 md:h-16 rounded-full transition-all ${
                          cell === null
                          ...
                          hoveredCol === colIdx && cell === null && isMyTurn && rowIdx === ROWS - 1
        ...
        {gameState.status === 'finished' && (
          <div className="space-y-6 text-center">
            <div className="text-6xl mb-4">
              {gameState.isDraw ? '🤝' : gameState.winner === user?.email ? '🏆' : '😢'}
            </div>
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
              {gameState.isDraw ? "It's a Draw!" : gameState.winner === user?.email ? 'You Won!' : 'You Lost!'}
            </h2>

            <button
              onClick={resetGame}
              className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg rounded-lg transition-colors flex items-center justify-center gap-2"
            >
