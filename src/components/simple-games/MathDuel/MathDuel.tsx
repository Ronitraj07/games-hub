...
import { RefreshCw, ArrowLeft, Zap, Trophy } from 'lucide-react';
...
const TIME_PER_Q = { easy: 15, medium: 12, hard: 8 };
...
export const MathDuel: React.FC = () => {
  ...
  if (!difficulty) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-900 dark:to-green-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          <div className="text-6xl mb-4">🧮</div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Math Duel</h1>
          <p className="text-gray-600 dark:text-gray-400 mb-8">Choose your difficulty level</p>
          <div className="grid gap-3">
            {(['easy', 'medium', 'hard'] as Difficulty[]).map(d => (
              <button
                key={d}
                onClick={() => { setDifficulty(d); nextQuestion(d); }}
                className={`p-4 rounded-xl font-semibold text-lg transition-all hover:scale-105 ${
                  d === 'easy' ? 'bg-green-500 hover:bg-green-600 text-white' :
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-900 dark:to-green-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <div className="flex gap-3">
            ...
            <Link to="/" className="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold py-3 rounded-xl transition flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-900 dark:to-green-950 p-4">
      <div className="max-w-lg mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">🧮 Math Duel</h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
          ...
          <div className="text-center py-8 mb-6">
            <p className="text-4xl font-bold text-gray-900 dark:text-white tracking-wide">
              {problem?.question}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {problem?.options.map((option, i) => {
              let style = 'bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 hover:border-green-400 hover:bg-green-50 dark:hover:bg-green-900/20';
              if (showFeedback) {
                if (option === problem.answer) style = 'bg-green-100 dark:bg-green-900/40 border-2 border-green-500';
                else if (option === selected) style = 'bg-red-100 dark:bg-red-900/40 border-2 border-red-500';
                else style = 'opacity-40 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600';
              }
              return (
                <button key={i} onClick={() => handleAnswer(option)} disabled={showFeedback}
                  className={`${style} rounded-xl py-5 text-2xl font-bold text-gray-800 dark:text-white transition-all hover:scale-105 active:scale-95 disabled:cursor-not-allowed`}>
                  {option}
                </button>
              );
            })}
