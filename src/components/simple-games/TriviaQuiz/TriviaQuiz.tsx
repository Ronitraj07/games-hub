...
export const TriviaQuiz: React.FC = () => {
  ...
  const handleAnswer = useCallback((optionIndex: number | null) => {
    ...
    if (isCorrect) { setScore(s => s + (timeLeft > 15 ? 15 : timeLeft > 8 ? 10 : 5)); playSound('success'); }
  ...
  const handleRestart = () => {
    ...
    setTimeLeft(TIME_PER_QUESTION); setGameOver(false);
  ...
  if (gameOver) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-blue-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={handleRestart} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-blue-950 p-4">
      <div className="max-w-lg mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">❓ Trivia Quiz</h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
          ...
          <div className="mb-2 flex justify-between">
            <span className="text-xs text-gray-400">Question {current + 1} of {TOTAL_QUESTIONS}</span>
            <span className="text-xs text-gray-400">{Math.round((current / TOTAL_QUESTIONS) * 100)}% done</span>
          </div>
          <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 mb-6">
            <div className="bg-indigo-400 h-1.5 rounded-full transition-all" style={{ width: `${(current / TOTAL_QUESTIONS) * 100}%` }} />
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            {q.options.map((option, i) => {
              let style = 'bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20';
              if (showResult) {
                if (i === q.answer) style = 'bg-green-50 dark:bg-green-900/30 border-2 border-green-500 text-green-700 dark:text-green-400';
                else if (i === selected) style = 'bg-red-50 dark:bg-red-900/30 border-2 border-red-500 text-red-700 dark:text-red-400';
                else style = 'bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 opacity-50';
              }
              return (
                <button
                  key={i}
                  ...
                  disabled={showResult}
                  className={`${style} rounded-xl px-4 py-3 text-left font-medium transition-all duration-200 flex items-center gap-3`}
                >
                  <span className="w-7 h-7 rounded-full bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 flex items-center justify-center text-sm font-bold text-gray-600 dark:text-gray-300 shrink-0">
                    {['A', 'B', 'C', 'D'][i]}
                  </span>
                  {option}
                </button>
