...
const WORDS = [
  ...
  'Candle','Compass','Dolphin','Umbrella','Lantern','Heart',
];
...
export const Pictionary: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { clearInterval(timer); setPhase('guessing'); setTimeLeft(30); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { clearInterval(timer); setResult('timeout'); setPhase('result'); return 0; }
        return prev - 1;
      });
    }, 1000);
    return () => clearInterval(timer);
  ...
  const startGame = () => {
    ...
    setTimeLeft(TIMER); setGuess(''); setResult(null); setPhase('drawing');
  };
  ...
  const clearCanvas = () => {
    ...
    const ctx = getCtx()!; ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, canvas.width, canvas.height);
  };
  const downloadDrawing = () => {
    const canvas = getCanvas(); if (!canvas) return;
    const link = document.createElement('a');
    link.download = `pictionary-${word}.png`; link.href = canvas.toDataURL(); link.click();
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {phase === 'idle' && (
          <div className="glass-card p-8 text-center">
            ...
            <div className="text-left bg-white/40 dark:bg-white/5 rounded-xl p-4 mb-6 space-y-2 text-gray-700 dark:text-gray-300">
              <p>1. 🎨 <strong>Drawer</strong> sees a secret word and draws it (90s)</p>
              <p>2. 👁️ <strong>Guesser</strong> then has 30 seconds to guess</p>
              <p>3. ⭐ Earn points based on how fast you guess!</p>
              <p>4. 🔄 Take turns each round!</p>
            </div>
            <button onClick={startGame} className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white font-bold py-3 px-8 rounded-xl transition text-lg">
              Start Drawing!
            </button>
          </div>
        )}
            ...
            <div className="glass-card p-4">
              <div className="flex justify-between items-center">
                <span className="font-medium text-gray-600 dark:text-gray-400">
                  {phase === 'drawing' ? `Round ${round} • Draw:` : `Round ${round} • Guess!`}
                </span>
                {phase === 'drawing' && <span className="text-lg font-bold text-pink-600 dark:text-pink-400">{word}</span>}
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                <div className="bg-gradient-to-r from-pink-500 to-rose-500 h-2 rounded-full transition-all"
                  style={{ width: `${phase === 'drawing' ? (timeLeft/TIMER)*100 : (timeLeft/30)*100}%` }} />
              </div>
            </div>

            <div className="glass-card overflow-hidden">
              <canvas ref={canvasRef} width={600} height={400}
                className="w-full cursor-crosshair touch-none" style={{ background: '#ffffff' }}
                ...
                <div className="flex gap-2 mb-3 flex-wrap items-center">
                  ...
                  <button onClick={() => setErasing(e => !e)}
                    className={`p-2 rounded-lg transition ${
                      erasing ? 'bg-blue-500 text-white' : 'glass-btn text-gray-700 dark:text-gray-300'
                    }`}>
                    <Eraser size={16} />
                  </button>
                </div>
        ...
        {phase === 'result' && (
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">Score: <strong>{score}</strong></p>
            <div className="flex gap-3 justify-center">
              <button onClick={nextRound} className="bg-gradient-to-r from-pink-500 to-rose-500 text-white font-semibold px-6 py-3 rounded-xl transition flex items-center gap-2">
                <RefreshCw size={18} /> Next Round
              </button>
              <Link to="/" className="glass-btn text-gray-700 dark:text-gray-300 font-semibold px-6 py-3 rounded-xl transition flex items-center gap-2">
