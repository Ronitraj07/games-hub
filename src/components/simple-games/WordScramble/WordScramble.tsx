...
const scramble = (word:string):string=>{
  const a=word.split(''); for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}
  return a.join('')===word?scramble(word):a.join('');
};

// Seeded shuffle so both players get the same word order
...
export const WordScramble: React.FC<{ sessionId?: string }> = ({ sessionId }) => {
  ...
  const handleNext=(correct:boolean)=>{
    const next=curIdx+1;
    if(correct){const pts=timeLeft>20?15:timeLeft>10?10:5;setScore(s=>s+pts+(streak>=2?5:0));setStreak(s=>s+1);playCorrect();}
  ...
  const activeScrambled=isOnlineActive?scramble(curOnlineWord?.word||''):scrambled;
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="glass-card p-6 mb-4">
          <div className="flex justify-between items-center mb-4">
            ...
            {!isOnlineActive&&<div className={`flex items-center gap-1 font-bold text-lg ${timerColor}`}><Clock size={18}/>{timeLeft}s</div>}
            {streak>=2&&<div className="flex items-center gap-1 text-orange-500"><Star size={16} fill="currentColor"/><span className="text-sm font-bold">{streak}x streak!</span></div>}
          </div>

          {!isOnlineActive&&<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-6">
          ...
          <div className={`text-center mb-6 p-6 rounded-xl transition-all ${
            feedback==='correct'?'bg-green-50 dark:bg-green-900/30':feedback==='wrong'?'bg-red-50 dark:bg-red-900/30':'bg-pink-50/50 dark:bg-pink-900/10'
          }`}>
          ...
          <form onSubmit={isOnlineActive?handleOnlineSubmit:handleSubmit} className="flex gap-2">
            ...
            <button type="submit" className="bg-gradient-to-r from-pink-500 to-purple-500 text-white font-semibold px-6 py-3 rounded-xl transition">Go!</button>
          </form>

          {!isOnlineActive&&(
            <button onClick={()=>setShowHint(true)} className="w-full mt-3 text-sm text-gray-500 hover:text-pink-600 transition">💡 Show Hint</button>
          )}
